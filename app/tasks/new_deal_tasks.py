...
from app.api.shopify.endpoints.update_customer import update_customer_metafields
...
@shared_task(name='app.tasks.new_deal_tasks.get_line_items_and_create_customers_v2')
def get_line_items_and_create_customers_v2(deal_id):
    hubspotLineItems = []
    variant_ids = []
    product_ids = []
    try:
        ...
        if primary_company:
            ...
            # Initialize draft_orders_data before the loop where it's used
            draft_orders_data = [
                {
                    "draftOrderId": [], # This will be populated later
                    "datePlaced": None, # This will be populated later
                    "totalPrice": None, # This will be populated later
                    "lineItems": [] # Initialize with an empty list
                }
            ]
            ...
            for to_object_id in line_item_ids:
                try:
                    ...
                    line_item = {
                        "lineItemTitle": product_title, # Assuming each item has a 'title' field
                        "lineItemQuantity": quantity, # Assuming each item has a 'quantity' field
                        "lineItemPrice": price # Assuming each item has a 'price' field
                        }
                    draft_orders_data[0]['lineItems'].append(line_item)
                    ...
                    metafields = prepare_metafields_for_graphql_products(product_info, product_data['hubspot_data'])
                    product_id, product_error = asyncio.run(create_shopify_product(hs_sku, product_title, metafields, discounted_price))
                    if product_error:
            ...
            deal_data = {
                ...
                "created_at": deal_info.get('properties', {}).get('createdate', ''),
        ...
        if customer_id:
            # Update the draft_orders_data without overwriting the list
            draft_orders_data[0]['draftOrderId'].append(draft_order_id)
            draft_orders_data[0]['datePlaced'] = deal_data['created_at']
            draft_orders_data[0]['totalPrice'] = float(deal_data['amount'])


        # Convert the draft orders data to a JSON string
        logger.info(f"Converting draft_orders_data to JSON: {draft_orders_data}")
        draft_orders_json = json.dumps(draft_orders_data)

        # Call the update_customer_metafields function to update the customer with the metafield
        updated_customer, update_error = asyncio.run(update_customer_metafields(customer_id, draft_orders_json)) 
        if update_error:
            error_message = f"Failed to update customer metafields: {update_error}"
            asyncio.run(send_error_to_webhook(error_message))
        else:
            logger.info(f"Successfully updated customer metafields for customer ID {customer_id}")
        ...
        # loop.run_until_complete(send_error_to_webhook(error_message))
        loop.close()
