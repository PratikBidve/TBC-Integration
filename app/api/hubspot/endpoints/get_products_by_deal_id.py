...
@router.get("/deals/{deal_id}/products", response_model=dict)
async def get_deal_products(deal_id: str, client=Depends(get_hubspot_client)):
    try:
        api_response = client.crm.associations.v4.basic_api.get_page(object_type="Deals", object_id=deal_id, to_object_type="products", limit=500)
